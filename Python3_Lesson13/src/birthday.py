...
def string_to_date(date):
    ...
    """
    try:
        # Create a ddatetime object from the date value
        formatter_string = "%m-%d-%Y"
        birthday = datetime.strptime(date, formatter_string)
    except ValueError as e:
        # log the format error then raise it again so it can e handled gracefully
        logging.error(e)
...
if __name__ == "__main__":
    ...
    parser.add_option('-b', '--birthday', dest="birthday", action="store", 
                      help="Your birthday in ;;-DD-YYYY format")
